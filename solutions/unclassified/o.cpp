...
void solve() {
	...
	auto nxt = [&](int i) -> int { return (i + 1) % n; };
	auto pre = [&](int i) -> int { return ((i - 1) % n + n) % n; };

	vi ans;
	auto xor_op_with_side_effect = [&](int x, int y) -> void { // [x, y) all xor arr[y]
		int cur = x;
		while (cur != y) {
			ans.push_back(cur);
			cur = nxt(cur);
	...
	while (l < r) {
		if (r - l <= n + l - r) {
			xor_op_with_side_effect(l, r);
			xor_op_with_side_effect(r, l);
			xor_op_with_side_effect(l, r);
		}
		else {
			xor_op_with_side_effect(r, l);
